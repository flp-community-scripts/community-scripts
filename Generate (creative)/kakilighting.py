...
from kakiutils import vecnorm, dotprod, interpolatePhenotypes
...
def lightMesh(mesh: mesh, lightVector: vec4, lightPheno: phenotype):
  ...
  for i in range(len(mesh.verts)):
    ...
    amt = min(max(dotprod(mesh.normals[i], lightVector) / vecnorm(mesh.normals[i]), 0), 1)
